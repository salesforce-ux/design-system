...
ask().then(async answers => {
  ...
  const pResults = prettier.format(data, {
    singleQuote: true,
    filepath: tmpFile
  });
  ...
  postcss([
    ...
    .then(async prefixedResult => {
      ...
      await fs.writeFile(outputFilename, prefixedResult.css).then(() => {
        console.log(chalk.white(`➜ CSS written to: ${outputFilename}`));
      });
      ...
      await postcss([
        ...
        .then(async minifiedResult => {
          ...
          await fs
            .writeFile(outputMinifiedFilename, minifiedResult.css)
            .then(() => {
              console.log(
                chalk.white(
                  `➜ Minified CSS written to: ${outputMinifiedFilename}`
                )
              );
            });
        });
