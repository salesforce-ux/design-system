...
<CodeView style={getDemoStylesById(Base.examples, 'with-user-initials-inversed')}>
...
## Sizes
Avatar Groups come in 4 different sizes: `x-small`, `small`, `medium`, and `large` by applying a `slds-avatar-group_{size}` modifier class on `slds-avatar-group`. If the size modifer class is not used, `medium` is also the default size.
