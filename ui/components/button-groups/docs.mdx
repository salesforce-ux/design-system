...
<CodeView>
  {ButtonGroupsBaseExamples.default}
</CodeView>
...
<Blockquote type="note" header="Mobile Only">
  The <code>.slds-button-group_stacked</code> modifier only affects mobile
  devices, all other devices will show the default layout.
</Blockquote>

<CodeView>
  {getDisplayElementById(ButtonGroupsBaseExamples.examples, 'touch-stacked')}
</CodeView>
...
See the [Base examples](#Examples) for various usecases such as [inverse](#Inverse).
