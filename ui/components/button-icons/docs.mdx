...
Button icons are button elements that represent their behavior with an [icon](/components/icon) instead of text.  The button itself should have the classes `slds-button` and `slds-button_icon`, while the icon has `slds-button__icon` on it.
