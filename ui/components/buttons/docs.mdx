...
<CodeView exampleOnly isMobile style={{}}>
  <React.Fragment>
    {ButtonExamples.default}
    {getDisplayElementById(ButtonExamples.examples, 'neutral')}
    {getDisplayElementById(ButtonExamples.examples, 'brand')}
...
<MobileNotice docsLink="#Mobile" header="Mobile context changes" elementName="buttons" />

<CodeView exampleOnly isMobile style={{ backgroundColor: '#16325c', padding: '0.5rem' }}>
  {getDisplayElementById(ButtonExamples.examples, 'inverse')}
</CodeView>
...
<Blockquote type="a11y" header="Accessibility Requirement">
  Be sure to include the attribute `aria-live="assertive"` on the button. The
  `aria-live="assertive"` attribute means the value of the `&lt;span&gt;` inside
