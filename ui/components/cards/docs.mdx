...
<CodeView exampleOnly>{CardExamples.default}</CodeView>
...
<CodeView>{CardExamples.default}</CodeView>
...
<Blockquote type="note" header="Styling Note">
  ...
  padding, use the <code className="doc">slds-card__body_inner</code> class on
  the <code className="doc">slds-card__body</code> element.
</Blockquote>
...
<Blockquote type="note" header="Implementation Note">
  The examples in this documentation use the `slds-card__footer-action` class on
  the footer link `&lt;a&gt;` to force the footer's click target to span the
