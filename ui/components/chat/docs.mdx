...
<Blockquote header="Use of aria-label" type="a11y">
  Each message meta data wrapper has <code className="doc">aria-label</code>{' '}
  ...
  . A screen reader, when <code className="doc">aria-label</code> is applied to
...
<Blockquote header="Aria Hidden Avatars" type="a11y">
  ...
  reason, we hide the avatar using `aria-hidden="true"` on the `&lt;span&gt;`
  with `class="slds-avatar"` to avoid redundancy for screen reader users.
</Blockquote>
...
<Blockquote header="Contrast" type="a11y">
  ...
  <code className="doc">slds-file__title_scrim</code> class in conjunction with{' '}
  <code className="doc">slds-file__title</code> to ensure proper contrast.
...
<Blockquote header="aria-hidden" type="a11y">
  ...
  <code className="doc">aria-hidden="true"</code> on the text they are currently
