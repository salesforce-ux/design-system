...
The <code>log</code> role has an implicit <code>aria-live</code> property value of <code>polite</code>, meaning that it comes for free. It also has an implicit <code>aria-relevant</code> property value of <code>additions text</code>. Together this means any additional text, or chat items added to the list will automatically be politely announced by a screen reader
