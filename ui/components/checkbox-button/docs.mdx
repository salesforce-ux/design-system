...
The checkbox `<input>` is visually hidden with `.slds-assistive-text` and a a faux checkbox is created using the [icon blueprint](/components/icons) which allows the display of an icon instead of the standard checkbox.
