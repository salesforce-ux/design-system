...
<CodeView frameStyles={{ height: '20rem' }} frameOnly hideDeviceSelector>
  {getDisplayElementById(BaseExamples.states, 'focused-open')}
</CodeView>
...
<MobileNotice docsLink="#Mobile" header="Mobile context changes" elementName="comboboxes with multiple selection" />

<CodeView frameOnly hideDeviceSelector>
  {getDisplayElementById(
    AutocompleteExamples.states,
    'lookup-multiple-options'
  )}
...
<MobileNotice docsLink="#Mobile" header="Mobile context changes" elementName="grouped comboboxes (cross-entity / polymorphic)" />

<CodeView frameStyles={{ height: '21rem' }} frameOnly hideDeviceSelector>{GroupedExamples.default}</CodeView>
