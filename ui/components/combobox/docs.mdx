...
<Blockquote type="a11y" header="Accessibility Requirement">
  When highlighting an option with "faux focus", the{' '}
  <code>aria-activedescendant</code> attribute on the `input` needs to be set to
  the value of the ID of the highlighted option. The highlighted option must
  also have <code>aria-selected="true"</code>.
</Blockquote>
...
<Blockquote type="a11y" header="Accessibility Requirement">
  ...
  attribute to the text <code>input</code>. Be sure to remove it again when the
  option is cleared.
</Blockquote>
...
<Blockquote type="a11y" header="Accessibility Requirement">
  Be sure to set <code>aria-controls</code> on the Object Switcher, which points
  to the Search Combobox ID.
</Blockquote>
