...
### Accessibility
To create an accessible table, the top row of column headers (<code>th</code>) are placed in a <code>thead</code>. Each one receives the <code>scope="col"</code> attribute. The first non-actionable (meaning that doesn't contain a checkbox or menu) column in each row should be marked as a th with a <code>scope="row"</code> attribute.
