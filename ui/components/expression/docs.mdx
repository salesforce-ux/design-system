  ...
  <ul>
    <li className="slds-expression__row">
      <fieldset>
        ...
        <div className="slds-expression__row_inputs">
        {...}
        </div>
      </fieldset>
    </li>
...
<CodeBlock toggleCode={false}>
  <legend className="slds-expression__legend"><span>AND</span><span className= "slds-assistive-text">Condition N</span></legend>
</CodeBlock>
...
- Focus should move to the first combobox in the new rule, labeled "Resource" or something similar
...
- Use Javascript to move focus to the delete button of the prior row.
...
<Example title="Expression Builder - Condition Added">
  <CodeView>
    <Expression optionSelected="all" resourceIsSelected>
      <ExpressionRow legendText="AND" conditionName="Condition 2" inputIsDisabled/>
    </Expression>
  </CodeView>
</Example>
...
<Example title="Expression Builder - Error State">
  <CodeView>
    <Expression optionSelected="all" resourceIsSelected>
      <ExpressionRow legendText="AND" conditionName="Condition 2" errorMessage="This field is required." hasError />
    </Expression>
  </CodeView>
</Example>
...
<Example title="Expression Builder - All Conditions">
  <CodeView>
    <Expression optionSelected="all" resourceIsSelected>
      <ExpressionRow legendText="AND" conditionName="Condition 2" resourceIsSelected/>
    </Expression>
  </CodeView>
</Example>
...
<Example title="Expression Builder - Any Conditions">
  <CodeView>
    <Expression optionSelected="any" resourceIsSelected>
      <ExpressionRow legendText="OR" conditionName="Condition 2" resourceIsSelected/>
    </Expression>
  </CodeView>
</Example>
  ...
  <CodeView>
    <Expression optionSelected="all" resourceIsSelected>
      <ExpressionGroup
        ...
        isGroup
      >
        <ExpressionRow conditionName="Condition 1" isGroup groupName="Condition Group 1" resourceIsSelected/>
        <ExpressionRow legendText="OR" conditionName="Condition 2" isGroup groupName="Condition Group 1" inputIsDisabled/>
      </ExpressionGroup>
    </Expression>
  </CodeView>
</Example>
...
<Example title="Expression Builder - Custom Logic">
  <CodeView>
    <Expression hasCustomLogic customLogicValue="1 AND 2" legendText="1" optionSelected="custom" resourceIsSelected>
      <ExpressionRow legendText="2" conditionName="Condition 2" inputIsDisabled/>
    </Expression>
  </CodeView>
</Example>
...
<Blockquote type="a11y" header="Accessibility Requirement">
  Be sure to use assistive text to improve the clarity of what the user might be editing. For example, add <code className="doc">&lt;span className="slds-assistive-text"&gt;Edit filter:</span></code> to each expression button element.
</Blockquote>
...
One example of in-context expression building is filtering. To see examples of filtering logic inside of a panel, checkout our [Panel](/components/panels/#Filtering-Panel) documentation

<Example title="Expression Builder - Filtering">
  <CodeView style={{maxWidth: '320px'}}>
    <Filters>
...
<Blockquote type="a11y" header="Accessibility Requirement">
  When an error message is displayed to the user, the <code className="doc">&lt;div&gt;</code> containing the error message should have the <code className="doc">role="alert"</code> attribute applied. This will immediately notify the user that an error has occurred.
</Blockquote>
