...
<Blockquote type="a11y" header="Accessibility Requirement">
  When an error message is displayed to the user, the <code>&lt;div&gt;</code>{' '}
  containing the error message should have the <code>role="alert"</code>{' '}
  attribute applied. This will immediately notify the user that an error has
  occurred.
</Blockquote>

<Blockquote type="a11y" header="Accessibility Requirement">
  The filter button that has an error is associated to that error with the use
  of <code>aria-describedby</code>. The value of <code>aria-describedby</code>{' '}
  matches the ID given to the element that contains the corresponding error
  message.
</Blockquote>
