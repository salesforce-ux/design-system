...
export let PostFooterActions = props =>
  <ul className="slds-post__footer-actions-list slds-list_horizontal">
    <li className="slds-col slds-item slds-m-right_medium">
      <button title="Like this item" className={classNames('slds-button_reset slds-post__footer-action', props.liked ? 'slds-is-active' : null)} aria-pressed={!!props.liked}>
        <SvgIcon className="slds-icon slds-icon-text-default slds-icon_x-small slds-align-middle" sprite="utility" symbol="like" />
        { props.liked ? 'Liked' : 'Like' }
      </button>
    </li>
    <li className="slds-col slds-item slds-m-right_medium">
      <button title="Comment on this item" className="slds-button_reset slds-post__footer-action">
        <SvgIcon className="slds-icon slds-icon-text-default slds-icon_x-small slds-align-middle" sprite="utility" symbol="share_post" /> Comment
      </button>
    </li>
    <li className="slds-col slds-item slds-m-right_medium">
      <button title="Share this item" className="slds-button_reset slds-post__footer-action">
        <SvgIcon className="slds-icon slds-icon-text-default slds-icon_x-small slds-align-middle" sprite="utility" symbol="share" /> Share
      </button>
    </li>
  </ul>;
  ...
  <header className="slds-post__header slds-media">
    ...
    <div className="slds-media__body">
      ...
      <p className="slds-text-body_small">
        <a href="javascript:void(0);" title="Click for single-item view of this post" className="slds-text-link_reset">5 days Ago</a>
      </p>
    </div>
  </header>;
      ...
      <ul className="slds-feed__list">
        <li className="slds-feed__item">
          <Post>
            <PostHeader />
            <PostContent>
              <h3 className="slds-text-heading_small">Barbecue Party Tips For A Truly Amazing Event?</h3>
              <p>Hey there! Here's the latest demo presentation <a href="javascript:void(0);" title="Jenna Davis">@Jenna Davis</a>, let me know if there are any changes. I've updated slides 3-8 and slides 16-18 slides with new product shots.</p>
            </PostContent>
            <PostFooter>
              <PostFooterActions />
