...
<Blockquote type="a11y" header="Accessibility requirement">
  ...
  <code>aria-describedby</code> attribute to the <code>&lt;input&gt;</code>. The
  `aria-describedby` attribute must reference the id of the error message. This
  configuration allows screen readers to read the associated error message when
  the invalid field is focused.
</Blockquote>
...
<Blockquote type="a11y" header="Accessibility Requirement">
  ...
  readers that this is a list by adding <code>role="list"</code> to the{' '}
