...
The logo region is a `<div>` with the class name `slds-global-header__logo` wrapped around a span with the company name. This inner span has the class name `slds-assistive-text` and is required to meet accessibility guidelines. The logo is applied as a background image to the div with CSS. This allows us to constrain the dimensions of the logo by a max-height of 40px and a max-width of 200px.
