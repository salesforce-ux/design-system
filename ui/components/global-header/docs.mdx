...
<Blockquote type="a11y" header="Accessibility Requirement">
  ...
  <code className="doc">aria-pressed</code> attribute to{' '}
  <code className="doc">true</code>. This will inform a screen reader that the
...
<div
  ...
  style={{ position: 'relative', height: '7.5rem' }}
>
...
<Blockquote type="a11y" header="Accessibility Note">
  ...
  <code className="doc">aria-hidden="true"</code> so the badge doesn't have to
  be inside the button itself. Since the badge count is hidden, we need to add
  the count number to the button text via some hidden assisitve text. The
  ...
  <code className="doc">aria-live="assertive"</code> to announce any count
  updates.
</Blockquote>
