...
You can see examples of the correct markup in the [Left Icon & Clear Button example](/input/?example=double-icon&variant=base) or the [Counter example](/input/?example=increment-decrement-counter&variant=base).
...
<CodeView>
  {getDisplayElementById(InputExamples.states, 'read-only')}
</CodeView>
...
<CodeView>
  {getDisplayElementById(InputExamples.examples, 'increment-decrement-counter')}
</CodeView>
