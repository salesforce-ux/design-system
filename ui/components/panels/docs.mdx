...
The `<h2></h2>` which contains the title of the panel requires the class `slds-panel__header-title`.
...
<Blockquote type="a11y" header="Accessibility Requirement">
  ...
  user inside the panel when it opens and returning them to the trigger when it
  closes.
</Blockquote>
...
<Blockquote type="note" header="Design Note">
  If the panel has been drilled into, then the panel displays an arrow facing to
  the left to indicate the direction the user drilled in from.
</Blockquote>

<Blockquote type="a11y" header="Accessibility Requirement">
  When the user drills in, the author must manage user focus by keeping the user
  inside the panel when it drills in and returning them to the drill-in trigger
  when it the user presses the back arrow.
</Blockquote>
...
<Example title="Panel base - Right dock drill in">
  ...
  >
    <Panel size="medium" title="Panel Header" docked="right" invoke="drill-in">
      A panel body accepts any layout or component
    </Panel>
  </CodeView>
</Example>
...
<CodeView style={{backgroundColor: '#fafaf9', position: 'relative', height: '600px', overflow: 'hidden'}}>
  <Panel size="medium" title="Filter" docked="right" invoke="toggle">
    <Filters>
