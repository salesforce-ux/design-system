...
The <code>.slds-progress-bar</code> accepts a range from 0% to 100%, and this percentage should be applied with inline styling to the <code>div</code> with class <code>slds-progress-bar\_\_value</code> using JavaScript. If implementing a horizontal progress indicator, set the width, otherwise set the height for vertical progress indicators.

### Accessibility

As the percentage of completion changes, be sure to update the <code>aria-valuenow</code> property on the `&lt;div&gt;` with `class="slds-progress-bar"`, as well as the `slds-assistive-text` `&lt;span&gt;`.
...
<Blockquote type="a11y" header="Accessibility Requirements">
  ...
  in the `&lt;span&gt;` with class `slds-assistive-text`, such as `Step 1 -
  Active`.
</Blockquote>
...
<Blockquote type="a11y" header="Accessibility Requirements">
  ...
  text in the `&lt;span&gt;` with class `slds-assistive-text`, such as `Step 2 -
  Completed`.
</Blockquote>
...
<Blockquote type="a11y" header="Accessibility Requirements">
  ...
  text in the `&lt;span&gt;` with class `slds-assistive-text`, such as `Step 3 -
  Error`.
</Blockquote>
...
<Blockquote type="note" header="Implementation Note">
  ...
  progress markers. Horizontal progress indicators use `&lt;button&gt;` for each
  step, whereas Vertical progress indicator steps are not interactive and
  therefore simply use `&lt;div&gt;`.
</Blockquote>
...
<Example title="Progress Indicator Vertical">
  <CodeView>
    <VerticalProgress value="25">
      ...
      <VerticalStep active>Step 2</VerticalStep>
      <VerticalStep>
        Step 3: Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do
